...
def get_benchmark_test_ids(href_links):
    return [link.split("/")[4] for link in href_links]
...
def get_cpu_vs_qps_context(df):
    ...
    return {
        'cpu_client_none_mtls_base': cpu_client_none_mtls_base,
...
def get_mem_vs_qps_context(df):
    ...
    return {
        'mem_client_none_mtls_base': mem_client_none_mtls_base,
        ...
        try:
            ...
            if not data[metric_name].head().empty:
                if data[metric_name].head(1).values == ['-']:
                    y_series_data.append('null')
                elif 'cpu' in metric_name or 'mem' in metric_name:
